...
% Adapted from code by Peter Perkins
% https://www.mathworks.com/matlabcentral/newsreader/view_thread/94209
...
[n,p] = size(x);
p= p-1; % The first column denotes the value
m1 = length(gridx1);
m2 = length(gridx2);
...
mu1(1,1,:) = x(:,2); mu1 = repmat(mu1,[m1,m2,1]);
mu2(1,1,:) = x(:,3); mu2 = repmat(mu2,[m1,m2,1]);
mu3(1,1,:) = x(:,1); mu3 = repmat(mu3,[m1,m2,1]);

f = sum( mu3.*normpdf(x1,mu1,bw(1)) .* normpdf(x2,mu2,bw(2)), 3);
