...
function [lnZ, alpha, mu, s] = varbvs_general (X, y, sigma, sa, logodds, eta, options)
  ...
  if isscalar(sa)
      sa = double(sa*ones(p,1));
  else
      sa = double(sa);
  end
  
  if isscalar(eta)
      eta = double(eta*ones(p,1));
  else
      eta = double(eta);
  end
  ...
  while true
    ...
    [alpha mu Xr] = varbvsupdate_general(X,sigma,sa,logodds,eta,xy,d,alpha,mu,Xr,I);
    ...
    if lnZ < lnZ0
      ...
      lnZ   = lnZ0;
      if verbose
        disp(['VLB decreased after ' num2str(iter) ' iterations'])
      end
      break
