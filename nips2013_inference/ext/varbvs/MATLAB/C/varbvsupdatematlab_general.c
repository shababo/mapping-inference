...
void mexFunction (int nlhs, mxArray* plhs[], 
  ...
  const DoubleVector eta     = getDoubleVector(prhs[10]);
  ...
  for (Index j = 0; j < m; j++) {
    ...
    // Perform the update.
    varbvsupdate_general(x,xy.elems[k],d.elems[k],sigma,sa.elems[k],logodds.elems[k],
		 alpha.elems+k,mu.elems+k,Xr.elems,n,eta.elems[k]);
  }
