...
void sample_X_D(
    ...
    for (n=0; n<N; n++) {
        ...
        for (t_ix=0; t_ix<T; t_ix++) {
            Y_n[t_ix] = 0;
            for (k=0; k<K; k++) {
                Y_n[t_ix] = Y_n[t_ix] + w_s[k]*X_s[n+N*k]*alpha_waveform(t[t_ix],D[n+N*k],tau,-gmax);
            }
        }
        ...
        for (k=0; k<K; k++) {
            ...
            logR_denom = 0; 
            for (t_ix=0; t_ix<T; t_ix++) {
                alpha_tmp = alpha_waveform(t[t_ix],D[n+N*k],tau,-gmax);
            ...
            if (odds_ratio == INFINITY || rand_unif() < odds_ratio/(1+odds_ratio)) {
                X_s[n+N*k] = 1;
                for (t_ix=0; t_ix<T; t_ix++) {
                    Y_n[t_ix] += w_s[k]*X_s[n+N*k]*alpha_waveform(t[t_ix],D[n+N*k],tau,-gmax);
                }
            } else {
                X_s[n+N*k] = 0;
            }
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) {
    ...
    /* Call the subroutine. */
    sample_X_D(X_s,D,N,K,T,Y,w_s,pi_nk,d_mean_nk,d_sigma_nk,t,tau,gmax,sigma_n);
}
