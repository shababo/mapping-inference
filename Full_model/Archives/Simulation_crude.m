...
%%
    ...
    output_warm= struct([]);
    for j = 1:num_threshold
        output_warm(j).alpha = .1*ones(n_cell+1,1);
        output_warm(j).mu = zeros(n_cell+1,1);
        output_warm(j).s_sq = ones(n_cell+1,1);
    end
...
%% Evaluation
overall_connectivity = zeros(n_cell,1);
    overall_connectivity_w = zeros(n_cell,1);
    
    overall_mark = zeros(n_cell,1);
    normalized_constants = zeros(n_cell,1);
    for j = 1:num_threshold
