...
assignments_current = cell(n_trial,1);
...
for i_trial = 1:n_trial
    ...
    assignments_current{i_trial,1} =mpp_new(i_trial).event_times;
    assignments_current{i_trial,1} =randsample(evoked_cell_index,length(assignments_current{i_trial,1}),true);
end
...
assignments_samples = cell(n_gibbs_sample,1);
...
while i_sample < n_gibbs_sample
    ...
    for i_trial_index = 1:n_trial_update
        ...
        n_events = length(assignments_current{i_trial,1});
        evoked_cell_index = evoked_cell{i_trial};
        firing_rates = zeros(length(evoked_cell_index)+1,1);
        size_rates = zeros(length(evoked_cell_index)+1,1);
        ...
        for i_event = 1:n_events
            ...
            assignments_current{i_trial}(i_event) = evoked_cell_index(find(mnrnd(1,chances)));
            
            this_cell = assignments_current{i_trial}(i_event);
            if this_cell > 0
                events_precell{this_cell} = [events_precell{this_cell} mpp_new(i_trial).amplitudes(i_event)];
            end
    ...
    if sigma_unknown == 1
        for i_cell = 1:n_cell
            this_cell_events = events_precell{i_cell};
            this_cell_n = length(this_cell_events);
            if this_cell_n > 0
                post_mean = (mu_m_hyper(i_cell)*mu_v_hyper(i_cell)+sum(this_cell_events))/...
                    (mu_v_hyper(i_cell)+this_cell_n);
                post_v = mu_v_hyper(i_cell)+this_cell_n;
                post_sigma_alpha = sigma_alpha(i_cell)+ this_cell_n/2;
                post_sigma_beta = sigma_beta(i_cell)+ (this_cell_n-1)*(std(this_cell_events)^2)/2+...
                    ((mean(this_cell_events)-mu_m_hyper(i_cell))^2/2)*(this_cell_n*mu_v_hyper(i_cell))/(this_cell_n+mu_v_hyper(i_cell));
            else
            ...
            % Draw mu:
            sigma_current(i_cell) = 1/gamrnd(post_sigma_alpha,post_sigma_beta);
    ...
    for i_cell = 1:n_cell
        ...
        post_alpha = gamma_alpha(i_cell)+length( events_precell{i_cell});
        post_beta = gamma_beta(i_cell) + expected_events-length( events_precell{i_cell});
       % Draw gamma:
       if expected_events < length( events_precell{i_cell})
    ...
    if i_counter > n_burnin 
        if mod(i_counter-1,n_skip) == 0  
            ...
            assignments_samples{i_sample}  = assignments_current;  
        fprintf('%d\n',i_sample);
        end
