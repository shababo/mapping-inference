...
x0 = [1 0.05];
Fsumsquares = @(x)sum((convo_mean(x,xdata) - ydata).^2);
...
% convo_param(2) gain
    ...
    % and Obtain a crude curve for SD (within condition variability)
     ...
     for i =1:length(xdata)
        [~, act_curr_index(i)]=min(abs(xdata(i) - spike_curves.current));
         var_explained(i)=spike_curves.sd(act_curr_index(i))^2+spike_curves.dev(act_curr_index(i));
     end
    ...
    delay_params.var=mean(var_vec(avai_index)-var_explained);
