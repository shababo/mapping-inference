function [V_vect, lambda,spikes] = lif_glm_sim_lbd(stim,params)
...
lambda(i) = params.linkfuncs{3}(V_vect(i)-params.V_thresh);

for t=dt:dt:t_end-1 %loop through values of t in steps of df ms        
    %V_inf = E_L + I_e_vect(i)*R_m;

    V_vect(i+1) = V_vect(i) + ...
        -params.membrane_resistance*V_vect(i) + stim(i)*params.optical_gain;%Euler's method
%fprintf('V %d, Stim %d\n', V_vect(i),stim(i));

%         lambda(i+1)=log(exp(V_vect(i+1)-v_threshold_sim)+1);
    lambda(i+1) = params.linkfuncs{3}(V_vect(i)-params.V_thresh);
