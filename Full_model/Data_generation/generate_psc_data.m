function [experiment_query, this_neighbourhood] = generate_psc_data(experiment_query,...
    experiment_setup,this_neighbourhood)
...
for i_group = 1:length(group_names)
    ...
    if any(get_group_inds(this_neighbourhood,group_names{i_group})) &&   isfield(experiment_query.(group_names{i_group}),'trials')
        ...
        if ~isempty(experiment_query.(group_names{i_group}).trials)
            ...
            [experiment_query_this_group, this_neighbourhood] = draw_point_processes(experiment_query_this_group, ...
