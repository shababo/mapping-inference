...
while ((n_trials < trial_max))
    ...
    for i_cell = 1:n_cell
        ...
        if gamma_path(i_cell,iter)>connected_threshold
            trials_powers( (i_cell-1)*K +(1:K),1)=...
               fire_stim_threshold./(pi_target(i_cell,loc_optimal(i_cell))*gain_samples(1:K,i_cell));
        else 
            trials_powers( (i_cell-1)*K +(1:K),1)=power_optimal(i_cell);
%randsample(power_level,K,true);
        end
    end
