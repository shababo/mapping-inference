...
mpp_naive_full=cell(0);
trials_locations_full=cell(0);
trials_powers_full=cell(0);
tic;
fitting_time_full=[];
...
%     draw samples of gains from the posterior distribution 
    ...
    for i_cell = 1:n_cell
        ...
        for k =1:K
            trials_powers( (i_cell-1)*K +k,1)=power_optimal(i_cell);
    ...
    mpp_full{iter}=mpp_temp;
    trials_locations_full{iter}=trials_locations;
    trials_powers_full{iter}=trials_powers;
...
%                  lklh_func=@lif_glm_firstevent_loglikelihood_for_VI;
    ...
    tend=toc;
    fitting_time_full(iter)=tend-tstart;
    ...
    [mean_gain_temp, var_gain_temp] = calculate_posterior_mean(...
        parameter_history.alpha_gain(:,end),parameter_history.beta_gain(:,end),...
        gain_bound.low,gain_bound.up);
...
%%
params_path_full=struct([]);

params_path_full(1).alpha=variational_params_path.alpha;
params_path_full(1).beta = variational_params_path.beta;
params_path_full(1).alpha_gain= variational_params_path.alpha_gain;
params_path_full(1).beta_gain= variational_params_path.beta_gain;

var_gamma_path_full=var_gamma_path;
var_gain_path_full=var_gain_path;
gamma_path_full=gamma_path;
gain_path_full=gain_path;
