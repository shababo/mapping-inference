...
n_round_digit=0;
%% Check initial values for the pair neighbours 
...
%% Run Gibbs sampler on these cells 
...
%% Find the cc that contains case_index
for i_cc = 1:n_cc
    if   sum(sum(cell_list{i_cc} == case_index ))>0
       selected_cc = i_cc; 
    end
end
%%
gain_initial_backup=gain_initial;
gamma_initial_backup=gamma_initial;
%% Balanced initialization 
gain_initial = gain_initial_backup;
gamma_initial=gamma_initial_backup;
...
gamma_balanced_fit=mean(gamma_samples{i_cc},2);
gain_balanced_fit=mean(gain_samples{i_cc},2);

%% Differentiable initialization 
gain_initial = gain_initial_backup;
gamma_initial=gamma_initial_backup;
gamma_initial(case_index)=[0.01 1];
...
gain_samples{i_cc}(:,1)=gain_current;
gamma_samples{i_cc}(:,1)=gamma_current;
...
gamma_imbalanced_fit=mean(gamma_samples{i_cc},2);
gain_imbalanced_fit=mean(gain_samples{i_cc},2);
%% The other way 

gain_initial = gain_initial_backup;
gamma_initial=gamma_initial_backup;
gamma_initial(case_index)=[1 0.01];
...
gamma_imbalanced_fit2=mean(gamma_samples{i_cc},2);
gain_imbalanced_fit2=mean(gain_samples{i_cc},2);

%% List the fits
gamma_balanced_fit(case_index) % 0.8 0.8 
gamma_imbalanced_fit(case_index) % 0.01 1
gamma_imbalanced_fit2(case_index) %1 0.01..

gain_balanced_fit(case_index) % 0.8 0.8 
gain_imbalanced_fit(case_index) % 0.01 1
gain_imbalanced_fit2(case_index) %1 0.01..

%% Turn the different initialization into matrices
gamma_fits =zeros(3,2);
gain_fits =zeros(3,2);

gamma_fits(1,:)= gamma_balanced_fit(case_index);
gamma_fits(2,:)= gamma_imbalanced_fit(case_index);
gamma_fits(3,:)= gamma_imbalanced_fit2(case_index);

gain_fits(1,:)= gain_balanced_fit(case_index);
gain_fits(2,:)= gain_imbalanced_fit(case_index);
gain_fits(3,:)= gain_imbalanced_fit2(case_index);
%% Obtain the probability and gamma 
prob_fits=cell([3 2]);

for i = 1:3
    for j= 1:2
        i_cell = case_index(j);
        stim_cell =unique(stimuli_size(relevant_trials_index ,i_cell));
        
        [~, i_gain]=min(abs(gain_fits(i,j)-gain_grid) );
        prob_fits{i,j}=zeros([length(stim_cell) length(current_template)]); 
        for k = 1:length(stim_cell)
            [~, i_stim] = min(abs(stim_unique-stim_cell(k)) );
            prob_fits{i,j}(k,:)= gamma_fits(i,j)*prob_trace{i_stim,i_gain};
           end
       end
end
%%
...
i=1;
 ...
 for k = 1:3
    prob_this_loc = prob_fits{j,1}(k,:);
    plot(1:n_grid,prob_this_loc*200,'col',colors(1),'LineStyle',style{k})
    hold on;
    prob_this_loc = prob_fits{j,2}(k,:);
    plot(1:n_grid,prob_this_loc*200,'col',colors(2),'LineStyle',style{k})
    hold on;
 end
