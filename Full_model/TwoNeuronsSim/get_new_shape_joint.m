function [new_shape_params]=get_new_shape_joint(parameter_current,new_locations,prior_info)
...
new_shape_params.new_locations=zeros(0,3);
n_new=0;
...
%% Calculate the joint distribution for the new shape and old shape (correlation matrix for the shapes) 
   ...
   new_shape_params.prior_mean = mean_3d;
   post_mean=mean_3d;
   tmp=parameter_current.shapes.mean;
   upb=parameter_current.shapes.bounds.up;
   lwb=parameter_current.shapes.bounds.low;
   
   post_mean(new_old_flags==0)=  (upb-lwb).*exp(tmp)./(1+exp(tmp))+lwb;
   new_shape_params.post_mean=post_mean;
    axis_list= fieldnames(GP_params);
        ...
        new_shape_params.Sigma_inv= inv(Full_Kcov);
        new_shape_params.Sigma_inv=(new_shape_params.Sigma_inv+new_shape_params.Sigma_inv')/2;
        Dmat=  diag([ diag(parameter_current.shapes.Sigma_inv).*exp(-parameter_current.shapes.log_sigma);  zeros(size(locs_new,1),1 )]);
        
        new_shape_params.Sigma=(Full_Kcov+Full_Kcov')/2;
        new_shape_params.Sigma_tilde_inv=new_shape_params.Sigma_inv+Dmat;
        new_shape_params.Sigma_tilde=inv(new_shape_params.Sigma_inv);
        new_shape_params.Sigma_tilde=(new_shape_params.Sigma_tilde+new_shape_params.Sigma_tilde')/2;
end


%% Calculate the parameters in the conditional distribution (correlation matrix for the shapes)
% 
% if ~isempty(  new_shape_params.locations)
%     locs_old=parameter_current.shapes.locations;
%     locs_new=  new_shape_params.locations;
