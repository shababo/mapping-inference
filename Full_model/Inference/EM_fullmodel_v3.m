...
function [gamma_path, mu_path, sigma_path, total_time, soft_assignments, background] = EM_fullmodel_v3(mpp, ...
...
%                 if (abs(weighted_sum(2)/sparsity_params.threshold- expected_events)/sparsity_params.eta)<1
%                     
%                     gamma_current(i_cell) =0;
%                 else
%                     gamma_current(i_cell)=min(1,weighted_sum(2)/(expected_events+sparsity_params.eta));
%                 end
                  if  (weighted_sum(2)/expected_events) < sparsity_params.eta
                      gamma_current(i_cell) =0; 
                  else
