%%%% Simulation: soft spike assignment + LIF-glm
%%%% spike train simulated via threshold-crossing LIF,
%%%% convolute spike with an 'assignment probability',
%%%% fitted with LIF-glm with logit link function.
...
%         V_vect(i+1) = V_vect(i) + (E_L-V_vect(i) + I_e_vect(i)*R_m)/tau*dt + sqrt(dt)*normrnd(stoc_mu,stoc_sigma);
        ...
        %if (V_vect(i+1)>V_th) %cell spiked
            prob_spike = min(1, exp((V_vect(i+1) - V_th)*dt) );
...
% for pid=1:3
%     subplot(1,3,pid);
%     imagesc(reshape(real_latency1st(pid,:),5,5)');
...
% for pid=1:3
%     subplot(1,3,pid);
%     imagesc(reshape(real_meanSp(pid,:),5,5)');
...
% for pid=1:3
%     subplot(1,3,pid);
%     imagesc(reshape(real_stdSp(pid,:),5,5)');
...
%     expg_k_loc(:,iloc)=expg_k(:).*ind_allLoc(:,iloc+1);
% end
% design=[ind1st(:).*expg_Vreset(:) expg_k(:) expg_k_loc];
% [betahat_conv_allLoc,~,stats_conv]=glmfit([ind1st(:).*expg_Vreset(:) expg_k(:) expg_k_loc],spTrain_allLoc,'Poisson','link','log');
...
%%
%%%% run R %%%%
% betahat_conv_allLoc=[-6.482574 -81.10257 0.01898234 0.01861527 0.01802396 0.01812253 0.0153087 0.01578668 0.01329207 0.01414961 0.01251234 0.01340983 0.01320243 0.01043094 0.01063751 0.007416529 0.007430213 0.00680272 0.00737882 0.006864403 0.0058602 0 0.00466871 0.003167881 0.001913522 0.001279969 0.0002794064];
...
%%%% compare true vs glmnn-fitted spatial weights
figure;
plot(gain_vec,'ro');
hold on
plot(k_vec,'g*');
