function [loglklh] = update_likelihood_dev2(stim_size,mpp,...
...
if ~isempty(stimulated_cells)
    prob_this_trial=zeros(length(stimulated_cells)+1,length(mpp(i_trial).intensity));
...
%         ts=toc;
        pdf_this_trial =   normpdf(1:length(mpp(i_trial).intensity),expectation,standard_dev);
...
%          te=toc;time_rec(7)=time_rec(7)+te-ts;
    ...
    prob_this_trial(end,:)=background_rate*ones(1,length(mpp(i_trial).intensity));
%     prob_this_trial(end,end)=background_rate*Tmax; % this is an approximation
    
else
    prob_this_trial=background_rate*ones(1,length(mpp(i_trial).intensity));
