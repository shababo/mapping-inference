function [M_intensity] = Intensity_v3(stimuli_size, n_stimuli_grid,n_grid_voltage,...
...
for i_cell = 1:n_cell_local
    ...
    % We can save a lot of time if V_th are the same across cells
    V_th = cell_params.V_th(i_cell);
    V_reset = cell_params.V_reset(i_cell);
    E_L=0;
    V_max = V_th+5;
    V_min= V_reset-5;
    ...
    %(V_max -V_min)*(0: (n_grid_voltage/2) )/(n_grid_voltage-1) +V_min;
    
    [~, index_reset] = min(abs(v_grid-V_reset));
    [~, index_rest] = min(abs(v_grid-E_L));
    stim_cell = stimuli_size(:,i_cell)*cell_params.gain(i_cell);
    stim_seq =stim_cell( stim_cell> k_minimum);
    % Obtain stimuli information on this cell
    if sum(stim_cell> k_minimum ) > 5 & length(unique(stim_seq)) >n_stimuli_grid
        ...
        for i_stimuli = 1:n_stimuli_temp
            ...
            for i_t = 2:n_grid_time
                ...
                for i_v = 1:n_grid_voltage
                    v_noise = v_grid-v_grid(i_v)-((E_L-v_grid(i_v))*cell_params.g(i_cell) +...
