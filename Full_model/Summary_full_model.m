...
%% Calculate summary statistics:
  % Normalized reconstruction error of connectivity
    NRE_conn = norm((local_neuron_amplitudes>0)*(1-evoked_params.failure_prob)-overall_connectivity)/norm( 1*(local_neuron_amplitudes(:,1)>0));
        NRE_mark= norm(local_neuron_amplitudes -overall_mark)/norm(local_neuron_amplitudes(:,1));
    
    [~,~,~,temp] = perfcurve(local_neuron_amplitudes>0,overall_connectivity ,1);
    AUC_conn = temp;
 

 NRE_conn_int = norm((local_neuron_amplitudes>0)*(1-evoked_params.failure_prob) -mean_gamma_known_mini_int)/norm( 1*(local_neuron_amplitudes(:,1)>0));
 NRE_mark_int= norm(local_neuron_amplitudes -mean_amp_known_mini_int)/norm(local_neuron_amplitudes(:,1));
