...
n_disconnected = size(local_connected,1) - n_connected;
...
[~,~,~,temp] = perfcurve(local_amplitudes>0,overall_connectivity ,1);
AUC_conn = temp;
[~,~,~,temp] = perfcurve(local_amplitudes>0,mean_gamma_mini_gibbs,1);
AUC_conn_mini_gibbs = temp;
[~,~,~,temp] = perfcurve(local_amplitudes>0,mean_gamma_EM,1);
AUC_conn_EM = temp;
