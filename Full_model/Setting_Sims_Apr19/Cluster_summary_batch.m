...
for i_setting = 1:10
    for design = 0:1
        for arg1 = 1:50
            ...
            load(strcat(flnm, 'Rep',num2str(arg1),'_Working.mat')); %,'output_working');
            ...
            %         working= cell(length(t_seq),1);
            %         for ind_t = 1:length(t_seq)
            %             overall_connectivity = zeros(n_cell_local,1);
            %             overall_mark = zeros(n_cell_local,1);
            %             for j = 1:num_threshold
            %                 overall_connectivity = max(overall_connectivity, ...
            %                     output_working{ind_t}(j).alpha(2:end));
            %                 for i_cell = 1:n_cell_local
            %                     if overall_connectivity(i_cell) == output_working{ind_t}(j).alpha(i_cell+1)
            %                         overall_mark(i_cell)= (output_working{ind_t}(num_threshold).threshold(j)+output_working{ind_t}(num_threshold).threshold(j+1))/2;
            %                     end
            %                 end
            %             end
            %             working{ind_t} = struct;
            %             working{ind_t}.mean_gamma = overall_connectivity;
            %             working{ind_t}.mean_mu = overall_mark;
            %         end
            %
            %
            ...
            %         Gibbs= cell(N,1);
            %         for i_batch = 1:N
            %             Gibbs{i_batch}.mean_gamma =  mean(output_Gibbs{i_batch}.gamma_samples, 1);
            %             Gibbs{i_batch}.mean_mu =  mean(output_Gibbs{i_batch}.mu_samples, 1);
            %             Gibbs{i_batch}.mean_sigma =  mean(output_Gibbs{i_batch}.sigma_samples, 1);
            %             %Gibbs{i_batch}.comptime = t_delta;
            %         end
